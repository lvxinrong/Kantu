...
| Main-agent worldview synthesis | `src/system/synthesis.ts` + `src/tools/system-synthesis.ts` | Persist evidence, hand off to the invoking DSH main agent, allow bounded per-project evidence retrieval, record provenance, archive every attempt, then accept one validated repair |
| System document validators | `src/protocol/validation.ts` + `src/system/artifacts.ts` | Deterministic heading, metadata, artifact, internal-domain/data-identifier redaction, diagram semantics, evidence, and semantic gate validation |
