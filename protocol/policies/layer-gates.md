...
A structurally valid draft may have `校验状态=PASSED` while remaining `下层门禁=BLOCKED`. ArchScope does not enter project analysis automatically after a system scan. The system-to-project transition is defined only by `archscope/contract/layer-state-machine/v1`.
