...
- 所有工程级细节继续保存在 `runs/<run-id>/system/evidence/<project-key>.json`；主文档只保留系统级事实和必要证据指针。
- `runs/<run-id>/system/relations.json` 是完整、不可裁剪的系统关系候选目录。主文档可以聚合核心关系，但不得把目录中的关系静默当作不存在；名称匹配关系必须保持候选状态。
- 主文档的边界说明必须明确指向 `system/relations.json`，第 20 节必须包含“完整关系目录已生成”自检行。
- 第 0 节的关系候选总数、内部/未解析数量、证据强度构成和关系类型构成属于插件拥有的机器字段，只能逐字引用注入的机器统计，不得由模型重新计数。正文若重复这些数字，必须与机器字段完全一致。
...
- 不输出密码、Token、API Key、Client Secret、私钥、Bearer Token、嵌入式账号密码或签名参数等真正秘密值。
- 服务名、内部域名、IP、路由、数据库/Schema/表、Topic、Queue 等架构标识属于本地事实，应原样保留以支持关系核对；主文档是否聚合取决于系统级语义边界，而不是脱敏。
- 不能仅凭域名、Maven 坐标、包名或数据库前缀断言组织归属；可以保留观察到的名称，但组织归属必须保持待确认。
- 文档不得写入 `/Users/...`、`/home/...`、`/workspace/...` 或 Windows 用户目录等本机绝对路径；扫描根统一表述为当前工作区或 `.`。
...
1. 核对工程注册表与索引/evidence 是否一一对应，并核对完整 relations 目录的计数、内部目标和证据路径，显式保留失败、跳过和越界记录。
2. 判断当前目录性质和扫描边界。
...
9. 识别主要基础设施类别及使用方；源码中的实例、地址和命名空间可以作为本地事实保留，但不得升级为生产启用结论，也不得包含真实凭据。
...
文档开头标题使用 `# 00-系统级事实底座`。第 0 节必须保留协议版本、事实版本、文档状态、证据状态、下层门禁、校验状态，以及关系候选总数、内部关系数、未解析关系数、内部关系构成、未解析关系构成、关系类型构成十二行机器字段。事实版本和关系字段由 ArchScope 确定性写入。第 14 节至少给出五条真正的跨工程可信结论。第 16 节必须完整出现 `阻断项目级`、`影响生产边界`、`影响入口链路`、`影响数据归属`、`可延后` 五类问题；若无真实阻断项，`阻断项目级` 对应的问题列必须写“当前无阻断项”，不得为了填模板虚构阻断。第 18 节必须覆盖工程、生产边界、入口链路、内部关系、外部依赖和数据归属六个评分维度。第 19 节必须覆盖工程、生产服务、系统入口、入口链路、内部关系、外部依赖和数据资产七类对象；内部关系与外部依赖行的总数/高/中/低必须分别使用机器统计中的内部/未解析及 DIRECT_SOURCE/CONFIGURATION/NAME_MATCH 数量。

各章节优先使用下面的系统级表格结构；某项没有证据时保留一行“待确认”，不要省略表格：

- 第 0 节：`契约字段 | 值 | 说明`，固定包含全部十二行机器字段；关系字段的值保留为注入值或占位，插件提交时会覆盖。
...
- 第 20 节：`自检项 | 结果 | 说明`，至少包含工程注册表已生成、每个工程都有索引状态、完整关系目录已生成、关键待确认问题已分级、证据仲裁已执行、凭据与秘密值已检查、源码存在与生产启用已区分、系统级边界未越界、项目级门禁。
