...
import { systemRelationMetricValues } from './relations.js'
...
function compactEvidenceRecord(record: ProjectSystemEvidence, projectCount: number, runId: string) {
  ...
  return {
    ...
    fullEvidenceArtifact: `runs/${runId}/system/evidence/${record.projectKey}.json`,
  }
}
...
export function systemSynthesisInputDigest(
  ...
  return sha256(JSON.stringify({ registry, indexes, evidence, relations }))
}
...
export function buildSystemSynthesisContext(
  ...
  evidence: SystemEvidenceBundle,
  relations: SystemRelationCatalog,
  options: Pick<Config, 'evidenceContextMode' | 'fullEvidenceMaxBytes'> = {},
): SystemSynthesisContext {
  const fullEvidence = JSON.stringify(evidence.records)
  const evidenceBytes = Buffer.byteLength(fullEvidence, 'utf8')
  const fullEvidenceMaxBytes = options.fullEvidenceMaxBytes ?? 524_288
  const configuredMode = options.evidenceContextMode ?? 'auto'
  const useFullEvidence = configuredMode === 'full'
    || (configuredMode === 'auto' && evidenceBytes <= fullEvidenceMaxBytes)
  const evidencePayload = useFullEvidence
    ? evidence.records
    : evidence.records.map(record => compactEvidenceRecord(record, registry.projectCount, runId))
  const evidenceMode = useFullEvidence ? 'FULL' : 'BOUNDED'
...
你不是证据采集 worker。不得重新扫描代码。只能使用下面给出的工程注册表、索引状态、完整系统关系候选目录和单工程证据，以及按需调用 archscope_get_system_project_evidence 补取的原始单工程 evidence；除此之外只允许调用 archscope_commit_system_synthesis。证据注入模式为 ${evidenceMode}（原始 evidence ${evidenceBytes} UTF-8 bytes，AUTO 全量阈值 ${fullEvidenceMaxBytes} bytes）；系统关系候选目录始终完整，不参与裁剪。若为 BOUNDED，可针对高影响关系、冲突或重复候选，每次用 1-8 个 projectKey 补取完整证据。不要为了追求全量而逐工程补取，也不得把未补取内容补成事实。

工作要求：
1. 阅读并执行完整的系统级指令、分析政策、证据政策、门禁政策和文档契约。
2. 统一工程名、服务名和别名，综合入口、能力、基础设施、数据资产、内部关系、外部依赖与冲突。必须逐项仲裁完整 relations 目录；DIRECT_SOURCE/CONFIGURATION/NAME_MATCH 是证据强度而非生产状态，所有关系默认运行态未确认。关系数量只能引用下方“ArchScope 机器关系统计”，不得自行重新计数；插件会确定性写入机器字段并拒绝矛盾数字。
...
7. 生成三个 Mermaid flowchart 源码，不要使用 Markdown 代码围栏，不要包含密码、Token、API Key、私钥、嵌入式账号密码或签名参数。架构标识可以保留，但图仍须保持系统级边界。每张图必须包含“实线=源码证据、虚线=待确认推断、不代表生产运行”的明确图例；候选或推断关系必须使用虚线。
8. 机器字段“协议版本、事实版本、文档状态、证据状态、下层门禁、校验状态”和自检中的“项目级门禁”由插件在提交时规范化；你仍须保留对应表格行。事实版本由插件在终态提交时分配。若第 16 节存在真实的“阻断项目级”问题，最终门禁必须为 BLOCKED；READY 时该分类只能写“当前无阻断项”。
...
10. 提交成功后，用中文向用户汇报系统世界观摘要、主要可信边界、校验结果和产物路径。只使用工作区相对路径；history.json 位于当前发布根 system/history.json，不在 runs/<run-id>/system 快照内。
...
===== ArchScope 机器关系统计（唯一数字事实，不得重算） =====
${JSON.stringify(relationMetrics)}
...
现在开始跨工程语义综合。不要把确定性分类候选直接当作结论；必须进行证据仲裁后再提交。`
  ...
  return {
    ...
    evidenceMode,
    evidenceBytes,
    fullEvidenceMaxBytes,
    relationCount: relations.totalCount,
    prompt,
  }
}
